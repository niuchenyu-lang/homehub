...
### Added
- 家庭日历：月视图网格，支持前后月导航，事件按日期渲染
- 事件管理：创建、编辑、删除事件，支持全天/非全天事件
- 事件类型：个人、家庭、任务、餐食、其他五类，支持颜色标记
- 事件弹窗：标题、时间、地点、成员分配、备注、颜色选择器
- 日历统计：按事件类型汇总本月事件数量
- events 数据表：支持重复规则（RRULE 预留字段）、与任务关联
- 日历 API：`GET/POST/PUT/DELETE /api/v1/events`，支持按时间范围筛选
